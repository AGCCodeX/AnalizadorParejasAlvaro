...
public class Errores extends JFrame 
{
	...
	public Errores() {
		...
		scroll.setBounds(52, 44, 259, 322);
	...
	public Errores(LexSem obj, Sintactico obj2){ //en este constructor resivimos un objeto de tipo lex que nos va a mostrar los errores lex que se encontraron
		setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
		setBounds(100, 100, 350, 410);
		contentPane = new JPanel();
		contentPane.setBorder(new EmptyBorder(5, 5, 5, 5));
		...
		{
			String cad="";
			for(int i=0;i<errors.length;i++) //solo recopilamos los errores Sintactico
			{
				cad+=errors[i]+"\n";
			}
			JTextArea.setText(JTextArea.getText()+cad);
		...
		{
			String cad="";
			for(int i=0;i<errorsem.length;i++) //solo recopilamos los errores Sintactico
			{
				cad+=errorsem[i]+"\n";
			}
			JTextArea.setText(JTextArea.getText()+cad);
